---
title: "Project 4 Description <br> Unsupervised Learning"
...
Unsupervised learning refers to learning problems where we have unlabelled data (e.g. variables $X$) without any response variable $y$ that can be used in a supervised set-up. Usually, unsupervised learning algorithm refers to the task of learning the latent structure e.g. group membership, from the data. This project considers different methods for one specific task under unsupervised learning - clustering. 

This document will show demonstration of two popular methods: $K$-means and Bayesian finite mixture model. Your goal would be to apply these methods for either a simulated data where the true cluster labels are known to you, but not to the algorithm and you'll test methods on their ability to learn the correct cluster memberships. If you do this, generate a data with many variables and assign labels based on some of the predictors and then try to cluster the points. 

Alternatively, you can also use a real data set from NCI60 (Cell lines for different cancer types) given in the real data section. 


The k-means examples are taken from the ISLR book website and the finite mixture model examples are taken from Matthew Stephens' website. 

# Approach 1: K-means clustering 

This simulated data has two true clusters, the first 25 belongs to a different class than the last 25. 
...
First we perform $K$-means clustering with $k = 2$. 
...
The k-means clustering algorithm perfectly clusters the data in this case. 

## Choosing $K$

However, in reality, we won't know if there are really two clusters, and we can do k-means with $K = 3$. 
...
This will split up one of the clusters into two spurious clusters. 


In general, choosing the number $K$ is a hard question to answer, and there's no consensus. Here is a paper that looks into this question:

1. Estimating the number of clusters using cross-validation
Wei Fu and Patrick O. Perry (https://arxiv.org/pdf/1702.02658.pdf)[https://arxiv.org/pdf/1702.02658.pdf]


## Multiple starts 

We can calculate the value of total within sum of squares that we wish to minimise. If we let $C_1, C_2, \ldots, C_K$ be the sets containing the indices for the $K$ clusters, and $W(C_K)$ be the amount by which observations within the same cluster differ from each other, then the total within cluster variation would be: 

$$
\text{minimize}_{C_1, \ldots, C_K} \{ \sum_{k=1}^{K} W(C_k) \}
$$

To run the kmeans() function in R with multiple initial cluster assignments,
we use the nstart argument. Note that `km.out$tot.withinss` is the total within-cluster sum of squares, which we seek to minimize by performing K-means clustering.
...
# Approach 2: Finite mixture model 
...
This is a clear bimodal distribution. The individual components are still normal. 
...
Here the individual components are normal but the population distribution is not (because it's not even symmetric). 
...
**Likelihood** If we observe samples $X_1, X_2, \ldots, X_n$ independently from this mixture, the complete likelihood for $\pi$ is:
...
This is basically a Multinomial with $n = 1$. Dirichlet is still conugate to this likelihood. We will assume that the prior on $(\pi_1, \ldots, \pi_K)$ is a Dirichlet with all $\alpha_i = 1$. (In some sense this is an extension of U(0,1) to $K$ dimensions)
...
Given the data and the cluster indicators, the $k^{th}$ mean does not depend on the other mean parameters. We can calculate the distribution intuitively.
...
Data distribution: Let the two mean parameters be $\mu_1 = -2$ and $\mu_2 = 2$ and $\sigma_1 = \sigma_2 = 1$ (known). 
...
Here we look at the NCI60 cell lines data and the goal is to see whethere the observations cluster into the different types of cancers. However, when clustering we'll not use the labels in the process anyhow. 

Before any analysis, people often standardize the variables to have
mean zero and standard deviation one. This step is optional and should be performed only if we want each gene to be on the same scale.
...
nci.labs[1:4]
table(nci.labs)
sd.data=scale(nci.data) ## standardize
```
