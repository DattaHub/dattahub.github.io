...
So, another way of calculating correlation is thiking of correlation as the covariance when the $X$ and $Y$ variables have been standardized. Three different ways of calculating the correlation are shown below. 

(Note that the denominator is $n-1$ which gives an unbiased estimator.)

```{r}
cov(scale(dat$ozone),scale(dat$radiation))

n <- length(dat$ozone)
1/(n-1)*sum(scale(dat$ozone) * scale(dat$radiation))
