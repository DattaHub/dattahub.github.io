...
1. Why nonparametric? Parametric vs. distribution-free inference; Pitman efficiency
2. Permutation tests: the unifying idea behind most of what follows
3. Bootstrap and resampling, confidence intervals, bootstrap hypothesis tests
4. Ranks and order statistics (just enough theory to understand what follows) 
5. One-sample and paired-sample tests: sign test, Wilcoxon signed-rank test
6. Two-sample tests: Mann–Whitney, Wilcoxon rank-sum, and permutation-based alternatives
7. Multiple-sample methods: Kruskal–Wallis and Friedman tests

## Outline of This Course — Part II

**More Advanced Topics**

1. Rank-based linear models — ranks inside `lm()`, Spearman and Kendall correlation
2. Goodness-of-fit tests: KS, Lilliefors, Anderson–Darling
3. Kernel Density Estimation and $k$-nearest neighbors
4. Bootstrap in depth : bootstrap in regression, the parametric bootstrap
5. Survival Analysis: Kaplan–Meier estimator and log-rank test
6. Implement all of the above in `R`, with and without packages

::: {.callout-note}
If you have taken CMDA 2006 with me, some of these will feel familiar, but the treatment will be faster and more theoretical throughout.
This is only a tentative list of topics and may change to accommodate this class.
:::
...
- Primary reference for concepts and homework:  
  **Nonparametric Statistical Inference, 5th ed.** — Gibbons & Chakraborti
- Other references:
...
![Illustration of the Central Limit Theorem — details in slide](art/clt-1.png){fig-alt="Details in slide" width="90%"}

## {.center}

![Statistical cartoon — details in slide](art/cartoon16.jpg){fig-alt="Details in slide" width="70%"}
...
![Kaplan-Meier citation count — details in slide](art/KM_cite.jpg){fig-alt="Details in slide" height="180px"}
...
:::: {.columns}
::: {.column width="50%"}
![Top 100 most cited papers — details in slide](art/top100_cited.jpg){fig-alt="Details in slide" width="90%"}
:::
::: {.column width="50%"}
![Cox's proportional hazard paper — details in slide](art/Stiegler.jpg){fig-alt="Details in slide" width="90%"}
:::
::::
...
# Parametric vs. Nonparametric — Details {background-color="#861F41"}
...
- The IQ scores of 18 girls were:  
  114, 81, 87, 114, 113, 87, 111, 89, 93, 108, 99, 93, 100, 95, 93, 95, 106, 108.
- **We do not assume the scores are Normal, but we do assume the scores are IID.**
...
- With correct assumptions (e.g. normality), parametric methods are more efficient — but often not by as much as you'd think.^[The large-sample efficiency of the Wilcoxon test vs. $t$-test is $3/\pi \approx 0.955$.]
...
Calculate the probability of observing 7 or fewer heads in 18 tosses of a fair coin — or just use R:
...
- P-value = $P(\text{successes} \leq 7 \mid H_0\text{: Median IQ} = 100) = 0.4807$
...
**Example:** John Arbuthnot (1710) recorded the number of male and female
christenings in London for each year from 1629 to 1710. The data are
available at: <https://www.openintro.org/data/index.php?data=arbuthnot>
...
**Question:** Is the proportion of male christenings significantly greater
than 0.5?
...
- **We do not assume any particular distribution, but we do assume
the yearly counts are IID.**

. . .

- **Changed Question:** Is the probability that males outnumber females
in a given year significantly greater than 0.5?

. . .

- Let $K$ = number of years in which male christenings exceeded female
christenings. Under $H_0: p = 0.5$, we expect $K \approx 41$ out of 82 years.
...
- I will show how many of these methods can be derived using ideas from **linear regression** — via the simple `lm()` function in R — after we have understood them as standalone methods.

- Some popular nonparametric procedures are **not alternatives** but standalone tools — for example, the **Bootstrap**, which we will learn later.
...
4. Median is a better representative than mean (e.g. income, grade distributions)
5. Usually simpler tests, less affected by departures from ideal assumptions — but limited in power and conclusions
:::
::: {.column width="50%"}
**Reasons for Parametric**

1. Large sample — statistic is approximately Normal (CLT)
2. Normality doesn't hold, but other parametric tests apply, or data can be suitably transformed
