---
title: "Quantile Importance Sampling <br> Normal-Normal"
...
for(i in 1:r){
  ...
  value = nested_sampling(mu1,sigma1,mu2,sigma2,N,tol = 1e-7)
  simu.grid = vertical.grid(length(value$phi),N,"e")
  
  # Lambda = sQ(simu.grid,Y)
  mc.nested = c(mc.nested,-trapz(simu.grid[-1],value$phi)) ## Trapezoidal rule (Yakowitz)
  
  mc.nested.rect = c(mc.nested.rect, -sum((value$phi)*diff(simu.grid)))
}
...
mean.qis <- mean(mc.qis); 
mean.naive <- mean(mc.naive)
...
mean.nested <- mean(mc.nested); 
mean.nested.rect <- mean(mc.nested.rect)
median.nested <- median(mc.nested); median.nested.rect <- median(mc.nested.rect)
mape.nested <- median(abs((mc.nested)-(trueZ))/(trueZ))
mape.nested.rect <- median(abs(((mc.nested.rect)-(trueZ))/(trueZ)))
rmse.nested <- sqrt((mean((mc.nested-trueZ)^2)))
rmse.nested.rect <- sqrt((mean((mc.nested.rect-trueZ)^2)))

perf <- rbind((cbind(mean.qis,mean.naive,mean.nested,mean.nested.rect)),
...
(norm.plt <- ggplot(mc.data, aes(MC, fill = method)) + 
