...
Complete problem 1 below and send the R codes or the compiled pdf to jyotishka@vt.edu. 
...
$$
Here $\alpha = 0$ is equivalent to a ridge penalty, $\alpha = 1$ is the Lasso penalty, and anything in between ($0 < \alpha < 1)$ is Elastic Net. 


## Data Generation 

We fix the regression parameter vector $\beta$ with a few non-zero elements and a design matrix ($X$ matrix) with large $p$, small $n$. Specifically, we chose:
...
The assumed covariance structure $Cov(X) = \Sigma = \{(\rho)^{|i-j|}\}_{1\le i,j \le p}$ will lead to a generated data where the covariance between $X_i$ and $X_j$ will decay as $i$ and $j$ move further away, i.e. as $|i-j|$ increases. This is called an $AR(1)$ structure. See the heatmap below:
...
2. Download the breast cancer data-set from the [Canvas website](https://canvas.vt.edu/courses/151354/files/folder/datasets) and load it on your workspace by typing the following command on R console: 

```{R, eval = F}
load("bc-tcga.Rdata")
```

3. You will now see a large matrix $X$ and a vector $y$ on your workspace. What are their dimensions?

4. Perform a 10-fold cross-validation on this data-set using the R function `cv.glmnet` from `glmnet` library. 

5. Plot the fitted object (simply calling `plot` on the cross-validated fit will give you a plot of mean-squared error versus $\log(\lambda)$). Also plot the solution path. 

6. Calculate the predicted value of $y[1:100]$ for both LASSO and Ridge and compare the mean squared errors. 
