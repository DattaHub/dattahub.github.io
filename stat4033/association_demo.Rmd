...
## Linear is the key {.smaller}

- Here $\rho_{X,Y} = 1$ only for a perfect monotonic 'linear' increasing relationship. 

```{r, echo = T,fig.width = 4, fig.height=4,fig.align='center'}
x = (1:7) ;y = x^4
cor(x,y)
```
- Correlation is not 1 but there is a perfect relationship, only non-linear. 
```{r, echo = F,fig.width = 5, fig.height=3,fig.align='center'}
plot(x,y,type="p")
```
